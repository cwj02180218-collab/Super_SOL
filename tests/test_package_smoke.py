...
def test_package_exports_version() -> None:
    assert fablized_sol.__version__ == "0.9.2rc1"


def test_readme_exposes_beginner_plugin_and_current_model_contract() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")

    assert "codex plugin marketplace add cwj02180218-collab/Super_SOL --ref v0.9.2-rc1" in readme
    ...
    assert "SUPER_SOL_QUALITY_MODE=selective" in readme
    assert "SUPER_SOL_BENCHMARK_GUARD=1" in readme
