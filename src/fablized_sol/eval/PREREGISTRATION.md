...
## Run identity and grade binding

Hash the complete task text, verification argv, grader argv, fixture paths, file modes, and file
contents. Bind those task digests together with this preregistration digest, model and effort cells,
arm design, retry limit, harness and SDK versions, and both full container image references into one
canonical versioned run digest. Session identities derive from that run digest without delimiter
concatenation. Every plan event records the provenance, and the external grade file must name the
same run digest. A mismatch fails closed.
