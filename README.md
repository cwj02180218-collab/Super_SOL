...
백그라운드 서비스가 없고 **추가 API 과금 호출 없음**이 기본값입니다. Super SOL은 새로운
모델이나 온톨로지가 아니라, Codex가 실제로 작업하고 검증하도록 돕는 품질관리 하네스입니다.

## 초보자용 5분 설치

Codex CLI에서 저장소 마켓플레이스를 추가하고 플러그인을 설치합니다.

```bash
codex plugin marketplace add cuj0218/Super-SOL --ref main
codex plugin add super-sol@super-sol
```

ChatGPT/Codex 데스크톱 앱을 다시 열고 새 작업을 시작합니다. 처음 한 번은 Codex가 로컬
훅을 보여주므로 내용을 확인하고 신뢰를 승인해야 합니다. 이후에는 평소처럼 요청하면 됩니다.
...
파일 변경을 관찰했는데 그 이후 성공한 테스트를 확인하지 못하면 한 번만 추가 검증을 요청하고,
그 뒤에는 무한 반복하지 않고 검증 미확인을 명시합니다.
...
알려진 live 평가 명령과 `api.openai.com` 직접 호출은 명시적 승인 없이는 로컬 훅이
...
일상 플러그인 사용에는 Python, Docker, API 키가 필요하지 않습니다. 아래 하네스는 모델별
성능·비용 가설을 실험할 때만 사용합니다.
...
모델과 effort는 세션 식별자와 모든 plan/start/finish 이벤트에 함께 기록됩니다.
...
플래그가 없으면 이미지와 API 키가 있어도 실행 디렉터리를 만들기 전에 거부합니다. live 평가는
일상 테스트나 CI에서 자동 실행되지 않습니다.

## 컨테이너 공급망

예제 verifier와 grader는 공식 `python:3.12-alpine`의 멀티플랫폼 digest를 고정합니다.
한 명령으로 두 이미지를 빌드하고 Critical/High CVE를 fail-closed로 검사한 뒤 SPDX 2.3
SBOM을 생성할 수 있습니다.
...
| arm, model, effort, 시간, token 사용량 | shadow stream | 아니요 |
| 숨겨진 grader 결과 | 별도 grader 및 외부 grade 파일 | 아니요 |
...
- Codex 훅은 일부 shell, `apply_patch`, MCP 경로를 관찰하지만 모든 도구 경로를 완전히
  가로채는 운영체제 보안 경계는 아닙니다.
- 플러그인 Stop gate는 관찰 가능한 `apply_patch` 변경과 구조화된 zero exit code 검증을
  기준으로 하며, 인식하지 못한 검증은 성공 증거로 세지 않습니다.
